...
class Dataset(FrozenDict):
    ...
    def get_stacked_observations(self, idxs, key='observations'):
        if self.frame_stack is None or self.frame_stack < 1:
            return jax.tree_util.tree_map(lambda arr: arr[idxs], self[key])
            
        initial_state_idxs = self.initial_locs[np.searchsorted(self.initial_locs, idxs, side='right') - 1]
        obs_stack = [] # Will be [ob[t - frame_stack + 1], ..., ob[t]].
        for i in reversed(range(self.frame_stack)):
            # Use the initial state if the index is out of bounds.
            cur_idxs = np.maximum(idxs - i, initial_state_idxs)
            obs_stack.append(jax.tree_util.tree_map(lambda arr: arr[cur_idxs], self[key]))
        
        # 채널(마지막 차원) 기준으로 Concatenation
        return jax.tree_util.tree_map(lambda *args: np.concatenate(args, axis=-1), *obs_stack)
    ...
    def sample(self, batch_size: int, idxs=None):
        ...
        if self.frame_stack is not None:
            # Stack frames.
            batch['observations'] = self.get_stacked_observations(idxs, 'observations')
            initial_state_idxs = self.initial_locs[np.searchsorted(self.initial_locs, idxs, side='right') - 1]
...
class GCDataset:
    ...
    def sample(self, batch_size: int, idxs=None, evaluation=False):
        ...
        # Goal Augmentation
        if not evaluation and self.dataset.p_aug is not None:
            if np.random.rand() < self.dataset.p_aug:
                self.dataset.augment(batch, ['value_goals', 'actor_goals'])
    ...
    def get_observations(self, idxs):
        ...
        return self.dataset.get_stacked_observations(idxs)
