...
class MACAgent(flax.struct.PyTreeNode):
    ...
    def bc_actor_loss(self, batch, grad_params, rng):
        ...
        target_flow_actions = self.sample_flow_actions(batch['observations'], goals=actor_goals, noises=noises)
        actor_actions = self.network.select('actor_onestep_flow')(
            batch['observations'], goals=actor_goals, actions=noises, params=grad_params
        )
        distill_loss = jnp.mean((actor_actions - target_flow_actions) ** 2)

        actor_loss = flow_loss + distill_loss
        actor_info = {
            'flow_loss': flow_loss,
    ...
    def critic_loss(self, batch, grad_params, rng):
        ...
        goal = None if batch['rollout_goals'] is None else batch['rollout_goals'][0]
    ...
    def sample_actions(
        ...
        n_goals = None if goals is None else jnp.repeat(jnp.expand_dims(goals, 0), num_samples, axis=0)

        n_noises = jax.random.normal(seed, (num_samples, *observations.shape[:-1], self.config['action_dim']))
        n_actions = self.network.select('actor_onestep_flow')(n_observations, goals=n_goals, actions=n_noises)
